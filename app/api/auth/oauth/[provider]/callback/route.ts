...
export async function GET(request: Request, { params }: { params: Promise<{ provider: string }> }) {
  ...
  const publicUrl = process.env.PUBLIC_SITE_URL?.replace(/\/$/, '') || reqUrl.origin
  if (!code || (provider !== 'google' && provider !== 'discord')) return NextResponse.redirect(new URL('/?oauth=error', publicUrl))
  const url = process.env.SUPABASE_URL?.replace(/\/$/, '')
  const anon = process.env.SUPABASE_ANON_KEY
  if (!url || !anon) return NextResponse.redirect(new URL('/?oauth=missing_config', publicUrl))
  try {
    ...
    if (!user) {
      ...
      let username = base
      for (let i=1; i<100; i++) { const taken = await dbQuery(`users?select=id&username=eq.${encodeURIComponent(username)}&limit=1`) as unknown[]; if (!taken.length) break; username = `${base}${i}`.slice(0,20) }
      const salt = crypto.randomBytes(16).toString('hex'), placeholder = crypto.randomBytes(32).toString('hex')
    ...
    const response = NextResponse.redirect(new URL('/?oauth=success', publicUrl))
    response.cookies.set('vv_session', signSession({id:user.id,username:user.username}), {httpOnly:true,secure:true,sameSite:'lax',path:'/',maxAge:60*60*24*30})
    return response
  } catch (error) {
    console.error('OAuth callback:', error)
    return NextResponse.redirect(new URL('/?oauth=error', publicUrl))
  }
}
