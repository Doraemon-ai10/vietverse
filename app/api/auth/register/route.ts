...
export async function POST(request: Request) {
  try {
    ...
    const verifiedUntil = verifiedEmails.get(key)
    if (!verifiedUntil || verifiedUntil < Date.now()) return NextResponse.json({error:'Hãy xác minh OTP trước.'},{status:400})
    ...
    verifiedEmails.delete(key); otpStore.delete(key)
    ...
    if(user?.id) response.cookies.set('vv_session',signSession({id:user.id,username:name}),{httpOnly:true,secure:process.env.NODE_ENV==='production',sameSite:'lax',path:'/',maxAge:60*60*24*30})
    return response
  } catch (e) {
